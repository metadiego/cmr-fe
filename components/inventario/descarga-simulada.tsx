...
export function DescargaSimuladaPanel({ productoId }: { productoId: string }) {
  ...
  React.useEffect(() => {
    ...
    listComponentes(productoId)
      .then((cs) => { if (vivo) setOpcionalIds(cs.filter((c) => c.optional).map((c) => c.id)); })
  ...
  const nombrePorId = React.useMemo(() => {
    ...
    for (const l of data?.lines ?? []) m.set(l.productId, l.name ?? l.sku ?? l.productId);
    return m;
  }, [data]);
                ...
                {data.avisos.map((a, i) => (
                  <div key={i} className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                    ...
                    <span>
                      {a.type === "duplicado"
                        ? t("aviso.duplicado", {
                            nombre: (a.productId && nombrePorId.get(a.productId)) || t("esteProducto"),
                            veces: a.veces ?? 2,
                          })
                        : a.type === "ciclo"
                          ? t("aviso.ciclo")
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {data.lines.map((l, i) => (
                      <LineaRow key={`${l.productId}-${i}`} linea={l} t={t} />
                    ))}
                  </tbody>
                </table>
              </div>
              ...
              <div className="rounded-md border border-dashed bg-muted/20 p-3">
                ...
                <ul className="space-y-0.5 text-sm text-muted-foreground">
                  {data.estimados.map((e, i) => (
                    <li key={i}>{e.name ?? e.sku ?? "—"}{e.quantity != null ? ` × ${e.quantity}` : ""}</li>
                  ))}
                </ul>
              </div>
            )}
    ...
    <tr className="align-top">
      <td className="py-1.5 pr-3">
        <div style={{ paddingLeft: depth * 16 }} className="flex flex-wrap items-center gap-x-2">
          ...
          <span className="font-medium">{linea.name ?? linea.sku ?? linea.productId}</span>
          {linea.technicalName && <span className="text-xs text-muted-foreground">· {linea.technicalName}</span>}
          {duplicado && (
      ...
      <td className="px-3 py-1.5 text-right tabular-nums">× {linea.quantity}</td>
      <td className="px-3 py-1.5"><ModoChip modo={linea.deductionMode} t={t} /></td>
      <td className="px-3 py-1.5 text-right tabular-nums text-muted-foreground">
        {linea.referenceCost != null ? money.format(linea.referenceCost) : "—"}
      </td>
    </tr>
  );
}
