            ...
            resumen.items.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2 md:hidden">
                  {resumen.items.map((r, i) => (
                    <div key={`m-${r.productoId}-${r.almacenId ?? ""}-${r.loteId ?? ""}-${i}`} className="cursor-pointer rounded-xl bg-card p-3 shadow-sm shadow-[rgba(16,32,64,0.06)] ring-1 ring-foreground/10 transition-shadow active:shadow-none" onClick={() => setDetalleDe({ productoId: r.productoId, nombre: r.nombre ?? r.sku ?? "—" })}>
          ...
          {vista === "consolidado" && consol && !cargando && (
            consol.items.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2 md:hidden">
                  {consol.items.map((r) => (
                    <div key={`m-${r.productoId}`} className="cursor-pointer rounded-xl bg-card p-3 shadow-sm shadow-[rgba(16,32,64,0.06)] ring-1 ring-foreground/10 transition-shadow active:shadow-none" onClick={() => setDetalleDe({ productoId: r.productoId, nombre: r.nombre ?? r.sku ?? "—" })}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
